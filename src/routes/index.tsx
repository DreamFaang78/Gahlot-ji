...
import ControllerScene, { type ControllerSceneHandle } from "../components/ControllerScene";
...
function Index() {
  ...
  // Feature cards (desktop only) — separate from storyRef so it can persist
  ...
  useLayoutEffect(() => {
    ...
    (async () => {
      ...
      const ctx = gsap.context(() => {
        ...
        // ─── Pinned controller story — 3-step scroll scene ─────────────────
        ...
        const buildStory = (pinDistance: number, withCards: boolean) => {
          ...
          // Proxy object for the progress value — GSAP scrubs this 0→1
          ...
          const tl = gsap.timeline({
            scrollTrigger: {
              ...
              onUpdate: (self) => {
                // Push scroll progress into the ControllerScene RAF loop
                storySceneRef.current?.setProgress(self.progress);
              },
            },
          });
          // Animate the proxy so GSAP's scrub easing is applied
          tl.to(proxy, { progress: 1, ease: "none", duration: 1 });
          // Background cross-fade: void → arena at 70% scroll
          tl.to(storyBgVoidRef.current,  { opacity: 0, duration: 0.25, ease: "power2.inOut" }, 0.65);
          tl.to(storyBgArenaRef.current, { opacity: 1, duration: 0.25, ease: "power2.inOut" }, 0.65);
          // Feature cards stagger in at 72% on desktop (separate ScrollTrigger)
          if (withCards) {
            const cardStart = "top+=" + String(pinDistance * 0.72) + "px top";
            gsap.to(".feature-card", {
              opacity: 1, y: 0,
              stagger: 0.04, duration: 0.2, ease: "power2.out",
              scrollTrigger: {
                trigger: storyRef.current,
                start: cardStart,
    ...
    <div ref={rootRef} className="relative bg-background text-foreground">
      ...
      <section id="features" ref={storyRef} className="relative w-full">
        <div ref={storyStageRef} className="relative h-screen w-full overflow-hidden">
          {/* ── Background layers ──────────────────────────────────────────── */}
          {/* Void: dark deep space — visible at start */}
          <div
            ref={storyBgVoidRef}
            aria-hidden
          ...
          />
          {/* Arena: photo bg — revealed as progress → 1 */}
          <div
            ref={storyBgArenaRef}
            aria-hidden
            className="absolute inset-0"
            style={{
              backgroundImage: `linear-gradient(to bottom, rgba(6,6,8,0.60), rgba(6,6,8,0.82)), url(${arenaImg})`,
          ...
          <div
            ...
            style={{
              paddingInline: "var(--gutter)",
              background: "linear-gradient(to bottom, rgba(6,6,8,0.80), transparent)",
            }}
          >
          ...
          {/* ── Main scene: controller + monitor + beam + particles ────────── */}
          <ControllerScene
            ref={storySceneRef}
            className="absolute inset-0 z-20"
          />

          {/* ── Feature cards — desktop only, stagger in at ~72% scroll ────── */}
          <div
            ref={featureCardsRef}
